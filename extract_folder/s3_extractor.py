...
from utils import clean_column_names, add_metadata, get_new_source_files, mark_source_files_as_processed, write_to_s3_parquet
...
def _list_files(prefix, suffixes = None):
    ...
    if suffixes:
        return [obj["Key"] for obj in contents if any(obj["Key"].endswith(s) for s in suffixes)]
    else:
...
def extract_customers(chunk_size=200_000):
    ...
    """
    logger.info("[1/3]: ....................... Extracting Customers from S3 ......................")
    prefix = "customers/"
    path = f"s3://{DEST_BUCKET}/staging/customers/"

    new_files = get_new_source_files(prefix, '.csv')
    if not new_files:
        logger.warning("************ No new customer files found in the bucket to process ***********")
        return pd.DataFrame()
    ...
    for file_info in new_files:
        ...
        for chunk_num, chunk in enumerate(_read_csv_from_s3(file_info['key'], chunk_size=chunk_size), 1):
            ...
            logger.info(f"------------------------- Streamed Chunk {chunk_num}: {row_count} rows")
    ...
    logger.info(f"Loaded {total_rows} records of customers data from {len(new_files)} new zfiles into our Data Lake (s3)......................")
...
def extract_call_logs():
    ...
    """
    logger.info("[2/3]: ....................... Extracting Call Logs from S3 ......................")
    ...
    if not new_files:
        logger.warning("*************** No new call logs files to process ****************")
        return pd.DataFrame()
    ...
    mark_source_files_as_processed(new_files, EXECUTION_DATE)
    logger.info(f"Loaded {len(df)} call logs from {len(new_files)} new source files)......................")
    return df
...
def extract_social_media():
    ...
    if not new_files:
        logger.warning("****************** No new social media files to process ********************")
        return 0
    ...
    for idx, file_info in enumerate(new_files):
        ...
        try:
            ...
            logger.info(f"----------------------------------- Wrote {rows_processed} rows. Total: {total_rows}")
    ...
    if total_rows > 0:
        mark_source_files_as_processed(new_files, EXECUTION_DATE)
        logger.info(f"SUCCESS: Loaded {total_rows} records from {len(new_files)} files ✓")
    else:
